  ...
  return (
    <section aria-labelledby="personalization-title" className="mt-8 rounded-2xl border border-line bg-ivory p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4">
        ...
        {usesUpload && <label className="grid gap-2 text-sm font-semibold text-navy">{config.fields.multipleImageUpload ? 'Imagens' : 'Imagem'}{isRequired('uploadedFiles') && ' *'}<input accept="image/jpeg,image/png,image/webp" className="block w-full rounded-xl border border-dashed border-gold-dark/50 bg-cream p-3 text-sm font-normal text-muted file:mr-4 file:rounded-full file:border-0 file:bg-navy file:px-4 file:py-2 file:text-xs file:font-semibold file:text-cream" multiple={config.fields.multipleImageUpload} onChange={(event) => handleFiles(event.target.files)} required={isRequired('uploadedFiles')} type="file" /><span className="font-normal text-xs leading-5 text-muted">JPG, PNG ou WebP, até 10 MB por arquivo. Simulação local: arquivos não são enviados.</span>{value.uploadedFiles?.length ? <span className="font-normal text-xs text-gold-dark">{value.uploadedFiles.length} arquivo(s) selecionado(s)</span> : null}</label>}
      </div>
