...
function CheckoutInput({ label, value, onChange, required = false, type = 'text', autoComplete, error }: CheckoutInputProps) {
  return <label className="grid gap-2 text-sm font-semibold text-navy">{label}{required && ' *'}<input aria-invalid={Boolean(error) && required && !value.trim()} autoComplete={autoComplete} className="min-h-12 rounded-xl border border-line bg-cream px-4 font-normal text-ink outline-none focus:border-gold-dark" onChange={(event) => onChange(event.target.value)} required={required} type={type} value={value} /></label>
}
